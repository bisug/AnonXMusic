...
- Layout: `melody/__main__.py` (boot/idle/shutdown) · `melody/core/` (bot, calls, mongo, youtube, providers, telegram, userbot) · `melody/plugins/` (auto-loaded via `all_modules`) · `melody/helpers/` (queue, thumbnails, inline, utils) · `melody/locales/*.json` (13 languages) · `config.py` · `tests/`.
...
- Checks: `uv run ruff check . --select=E9,F63,F7,F82` and `uv run python -m unittest discover -s tests -v` (also CI: `.github/workflows/ci.yml` + docker build).
